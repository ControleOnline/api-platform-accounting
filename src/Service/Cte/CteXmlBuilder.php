...
class CteXmlBuilder
{
    public function build(array $invoices, array $fiscal, string $cfop, array $extra = []): string
    {
        ...
        $tomadorCode = $this->requiredOption($extra['tomador'] ?? null, 'Tomador', ['0', '1', '2', '3']);
    ...
    private function buildIde(array $invoices, array $fiscal, string $cfop, array $extra): \stdClass
    {
        ...
        $ide->retira = '1';
        $ide->indIEToma = $this->tomadorIeIndicator($this->tomador($first, $extra));
        return $ide;
    }
    ...
    private function tomador(InvoiceTax $invoice, array $extra): ?People
    {
        return match ((string) ($extra['tomador'] ?? '')) {
